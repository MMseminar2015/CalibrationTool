...
double StereoMatching::StereoCalibrate_bySugimoto(std::vector<cv::Mat> leftvec, std::vector<cv::Mat> rightvec)
{
	if (leftvec.size() != rightvec.size())//左右で画像数が違う場合
		return -1.0;
	...
	int k = 0;
	int numBoards = 0;
	for (int i = 0; i<undLeftvec.size(); i++)
	{
		cv::Mat gray1, gray2;

		cvtColor(undLeftvec[i], gray1, CV_BGR2GRAY);
		cvtColor(undRightvec[i], gray2, CV_BGR2GRAY);

		//チェッカーボード探し・キャリブレーション描画
		found1 = findChessboardCorners(undLeftvec[i], BoardSize, corners1, CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
		found2 = findChessboardCorners(undRightvec[i], BoardSize, corners2, CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
		if (found1) {
			cornerSubPix(gray1, corners1, Size(11, 11), Size(-1, -1), TermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 30, 0.1));
			drawChessboardCorners(gray1, BoardSize, corners1, found1);
		}
		if (found2) {
			cornerSubPix(gray2, corners2, Size(11, 11), Size(-1, -1), TermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 30, 0.1));
			drawChessboardCorners(gray2, BoardSize, corners2, found2);
		}
		//キャリブレーション結果表示
		imshow("image1", gray1);
		imshow("image2", gray2);
	...
	stereoCalibrate(object_points, imagePoints1, imagePoints2,
		CM1, D1, CM2, D2, undLeftvec[0].size(), R, T, E, F,
		cvTermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 100, 1e-5),
		CV_CALIB_SAME_FOCAL_LENGTH | CV_CALIB_ZERO_TANGENT_DIST);

	Mat R1, R2, P1, P2, Q;
	stereoRectify(CM1, D1, CM2, D2, undLeftvec[0].size(), R, T, R1, R2, P1, P2, Q);

	Mat map1x, map1y, map2x, map2y;
	Mat imgU1, imgU2;
	initUndistortRectifyMap(CM1, D1, R1, P1, undLeftvec[0].size(), CV_32FC1, map1x, map1y);
	initUndistortRectifyMap(CM2, D2, R2, P2, undLeftvec[0].size(), CV_32FC1, map2x, map2y);
